...
export class ImportService {
    ...
    public static readonly PARSE_ERROR_LINE_EXTRACTION_REGEX = '(?:L|l)ine.*?(\\d+).*?(?:C|c)olumn.*?(\\d+)';
    public static readonly DEFAULT_ROLE_DEFAULT_VALUE = false;
    public static readonly ANONYMOUS_ROLE_DEFAULT_VALUE = false;

    constructor(protected importUtils: ImportUtils = new ImportUtils(), protected legacyImportService: LegacyImportService = new LegacyImportService()) {
    }
    ...
    public parseFromXml(txt: string): PetriNetResult {
        ...
        if (txt.trim().endsWith(`</document>`)) {
            return this.legacyImportService.importFromXml(doc, result);
        }
    ...
    public importData(modelResult: PetriNetResult, xmlDoc: Document): void {
        for (const xmlData of Array.from(xmlDoc.getElementsByTagName('data'))) {
            try {
                ...
                if (scopeValue !== '') {
                    data.scope = scopeValue as ResourceScope;
                }
    ...
    public importTransitions(modelResult: PetriNetResult, xmlDoc: Document): void {
        for (const xmlTrans of Array.from(xmlDoc.getElementsByTagName('transition'))) {
            try {
                ...
                if (scopeValue !== '') {
                    trans.scope = scopeValue as ResourceScope;
                }
                trans.scope = (xmlTrans.getAttribute('scope') ?? ResourceScope.USECASE) as ResourceScope;
    ...
    importPlace(modelResult: PetriNetResult, xmlPlace: Element) {
        ...
        if (scopeValue !== '') {
            place.scope = scopeValue as ResourceScope;
        }
    ...
    public parseArc(result: PetriNetResult, xmlArc: Element): Arc<NodeElement, NodeElement> {
        ...
        if (scopeValue !== '') {
            arc.scope = scopeValue as ResourceScope;
        }
